...
import net.runelite.client.plugins.microbot.pumstersreflection.gson.HookInfo;
import net.runelite.client.plugins.microbot.pumstersreflection.gson.Hooks;
import net.runelite.client.plugins.microbot.pumstersreflection.rlapi.WidgetInfoPlus;
...
import static net.runelite.client.plugins.microbot.pumstersreflection.methods.MiscUtilities.sendGameMessage;
import static net.runelite.client.plugins.microbot.pumstersreflection.methods.PrayerInteractions.activatePrayer;
import static net.runelite.client.plugins.microbot.pumstersreflection.methods.PrayerInteractions.deactivatePrayer;
import static net.runelite.client.plugins.microbot.pumstersreflection.rlapi.PrayerExtended.getPrayerWidgetId;

@Slf4j
@Singleton
@PluginDescriptor(
        name = PluginDescriptor.Pumster + "Relection Lib",
        enabledByDefault = true,
        description = "Test utils for reflection",
        tags = {"testing", "reflection", "utilities", "utils", "plugin"}
)
public class KotoriUtils extends Plugin {
    ...
    {
        if (event.getMenuOption().contains("Test - Activate "))
        {
            sendGameMessage("Menu Insertion Test is a success. Thick Skin prayer activated.");
        }
        if (event.getMenuOption().contains("<col=39ff14>Test - Cast Fire Bolt</col> -> "))
        {
            sendGameMessage("Spell Selection Test is a success. Casted Fire Bolt.");
        }
    }
    ...
    private void testWalkHooks()
    {
        ...
        sendGameMessage("Current Location: " + currentPlayerLocation);
        sendGameMessage("Walking to: " + walkingPoint);
    }
    ...
    private void testInvokeHook()
    {
        ...
        if (client.isPrayerActive(prayer))
        {
            ...
            sendGameMessage("Kotori Utils Test - Deactivating Thick Skin prayer");
        }
        else
        {
            activatePrayer(prayer);
            sendGameMessage("Kotori Utils Test - Activating Thick Skin prayer");
        }
    ...
    private void testNpcAnimationHook()
    {
        ...
        if (actor instanceof NPC)
        {
            ...
            sendGameMessage("Kotori Utils Test - NPC Name: " + npcName + ", Animation ID: " + animationId);
        }
    }
    ...
    private void testNpcOverheadIconHook()
    {
        ...
        if (actor instanceof NPC)
        {
            ...
            if (headIcon == null)
            {
                sendGameMessage("Kotori Utils Test - NPC Name: " + npcName + " has no overhead icon.");
            }
            else
            {
                sendGameMessage("Kotori Utils Test - NPC Name: " + npcName + ", Overhead Icon: " + headIcon.name());
            }
        }
    }
    ...
    private void testMenuEntryHooks()
    {
        ...
        ReflectionLibrary.insertMenuEntry(index, "Kotori Utils Test - Activate ", "Thick Skin Prayer", MenuAction.CC_OP.getId(), 1, -1,
                getPrayerWidgetId(Prayer.THICK_SKIN), -1);
    }
    ...
    private void testIsMovingHook()
    {
        ...
        sendGameMessage("isMoving? " + ReflectionLibrary.areYouMoving() + ", pathLength: " + pathLength +
